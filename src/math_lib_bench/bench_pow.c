...
void init_pow(int N, double *__restrict a, double *__restrict b)
{
    for (int i = 0; i < N; i++) {
        a[i] = 1 + ((double) i) / N;
        b[i] = 0;
...
#pragma GCC optimize("-fno-tree-vectorize")
void bench_pow_scalar(int N, double *__restrict a, double *__restrict b)
{
    #pragma clang loop vectorize(disable)
    #pragma novector
    for (int i = 0; i < N; i++) {
...
void bench_pow_omp_simd(int N, double *__restrict a, double *__restrict b)
{
#ifdef VECTOR_LENGTH
    #pragma omp for simd simdlen(VECTOR_LENGTH)
#else
...
void bench_pow_vforce(int N, double *__restrict a, double *__restrict b)
{
    vvpow(b, a, a, &N);
}
#endif

void bench_pow(int N)
{
    printf("Benchmarking math function pow(). Array length: %d\n", N);
...
void bench_pow_rep(int N, int repetitions)
{
    printf("Benchmarking math function pow(). Array length: %d. Number of repetitions: %d\n", N, repetitions);
