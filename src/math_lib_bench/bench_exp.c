...
void init_exp(int N, double *__restrict a, double *__restrict b)
{
    for (int i = 0; i < N; i++) {
        a[i] = 1 + ((double) i) / N;
        b[i] = 0;
...
#pragma GCC optimize("-fno-tree-vectorize")
void bench_exp_scalar(int N, double *__restrict a, double *__restrict b)
{
    #pragma clang loop vectorize(disable)
    #pragma novector
    for (int i = 0; i < N; i++) {
...
void bench_exp_omp_simd(int N, double *__restrict a, double *__restrict b)
{
#ifdef VECTOR_LENGTH
    #pragma omp for simd simdlen(VECTOR_LENGTH)
#else
...
void bench_exp_vforce(int N, double *__restrict a, double *__restrict b)
{
    vvexp(b, a, &N);
}
#endif

void bench_exp(int N)
{
    printf("Benchmarking math function exp(). Array length: %d\n", N);
...
void bench_exp_rep(int N, int repetitions)
{
    printf("Benchmarking math function exp(). Array length: %d. Number of repetitions: %d\n", N, repetitions);
