...
void init_expm1(int N, double *__restrict a, double *__restrict b)
{
    for (int i = 0; i < N; i++) {
        a[i] = ((double) i) / N;
        b[i] = 0;
...
#pragma GCC optimize("-fno-tree-vectorize")
void bench_expm1_scalar(int N, double *__restrict a, double *__restrict b)
{
    #pragma clang loop vectorize(disable)
    #pragma novector
    for (int i = 0; i < N; i++) {
...
void bench_expm1_omp_simd(int N, double *__restrict a, double *__restrict b)
{
#ifdef VECTOR_LENGTH
    #pragma omp for simd simdlen(VECTOR_LENGTH)
#else
...
void bench_expm1_vforce(int N, double *__restrict a, double *__restrict b)
{
    vvexpm1(b, a, &N);
}
#endif

void bench_expm1(int N)
{
    printf("Benchmarking math function expm1(). Array length: %d\n", N);
...
void bench_expm1_rep(int N, int repetitions)
{
    printf("Benchmarking math function expm1(). Array length: %d. Number of repetitions: %d\n", N, repetitions);
