...
#include "JT21_multistep_cell_time_cell.h"
#include "cellmodel.h"
...
{
    // spread work across threads in outer loop over (chunks of) cells
    #pragma omp parallel for
    for (long v = 0; v < num_cells; v += MULTISTEP_VECTOR_LENGTH) {
        long v_len = MULTISTEP_VECTOR_LENGTH;
        // ensure that v + v_len <= num_cells
        if (v + v_len > num_cells) {
            v_len = num_cells - v;
        }
        double t = t_start;
        for (int ti = 0; ti < num_timesteps; ti++) {
            // annotate inner loop over chunk/vector with simd hints
#if defined(HINT_CLANG_SIMD)
#pragma clang loop vectorize(assume_safety)
#elif defined(HINT_OMP_SIMD)
#ifdef VECTOR_LENGTH
            #pragma omp simd aligned(states : CELLMODEL_STATES_ALIGNMENT_BYTES) simdlen(VECTOR_LENGTH)
#else
            #pragma omp simd aligned(states : CELLMODEL_STATES_ALIGNMENT_BYTES) simdlen(MULTISTEP_VECTOR_LENGTH)
#endif // defined(VECTOR_LENGTH)
#endif
            for (long i = v; i < v + v_len; i++) {
                // Assign parameters
                const cellmodel_float_t g_Na = parameters[PARAM_g_Na * padded_num_cells + i];
                const cellmodel_float_t lambda_Na =
                        parameters[PARAM_lambda_Na * padded_num_cells + i];
                ...
                const cellmodel_float_t lambda_K =
                        parameters[PARAM_lambda_K * padded_num_cells + i];
                ...
                const cellmodel_float_t Q10SLCaP =
                        parameters[PARAM_Q10SLCaP * padded_num_cells + i];
                ...
                const cellmodel_float_t alpha_RyR =
                        parameters[PARAM_alpha_RyR * padded_num_cells + i];
                const cellmodel_float_t beta_RyR =
                        parameters[PARAM_beta_RyR * padded_num_cells + i];
                const cellmodel_float_t eta_RyR = parameters[PARAM_eta_RyR * padded_num_cells + i];
                const cellmodel_float_t gamma_RyR =
                        parameters[PARAM_gamma_RyR * padded_num_cells + i];
                ...
                const cellmodel_float_t B_tot_sl =
                        parameters[PARAM_B_tot_sl * padded_num_cells + i];
                ...
                const cellmodel_float_t k_off_sl =
                        parameters[PARAM_k_off_sl * padded_num_cells + i];
                ...
                const cellmodel_float_t lambda_B =
                        parameters[PARAM_lambda_B * padded_num_cells + i];
                const cellmodel_float_t lambda_B_c =
                        parameters[PARAM_lambda_B_c * padded_num_cells + i];
                const cellmodel_float_t alpha_d_c =
                        parameters[PARAM_alpha_d_c * padded_num_cells + i];
                const cellmodel_float_t alpha_n_s =
                        parameters[PARAM_alpha_n_s * padded_num_cells + i];
                ...
                const cellmodel_float_t hLss =
                        FP_LITERAL(1.0)
                        / (FP_LITERAL(1.)
                           + FP_LITERAL(124658.506952)
                                     * Exp(FP_LITERAL(2.) / FP_LITERAL(15.) * V_m));
                ...
                const cellmodel_float_t sigma = FP_LITERAL(-0.142857142857143)
                                                + Exp(Nao / FP_LITERAL(67.)) / FP_LITERAL(7.);
                ...
                const cellmodel_float_t alpha_Xr1 =
                        FP_LITERAL(450.0)
                        / (FP_LITERAL(1.0)
                           + FP_LITERAL(0.0111089965382) * Exp(FP_LITERAL(-0.1) * V_m));
                ...
                const cellmodel_float_t xsss =
                        FP_LITERAL(1.0)
                        / (FP_LITERAL(1.)
                           + FP_LITERAL(0.76228973079) * Exp(-V_m / FP_LITERAL(14.)));
                const cellmodel_float_t tauxs =
                        FP_LITERAL(990.)
                        / (FP_LITERAL(1.)
                           + FP_LITERAL(0.842460441617) * Exp(-V_m / FP_LITERAL(14.)));
                ...
                const cellmodel_float_t I_K1 = FP_LITERAL(0.430331482912) * g_K1 * lambda_K
                                               * sqrt(Ko) * (-ek + V_m) * K1ss;
                ...
                const cellmodel_float_t I_CaL = lambda_c_d * pow(Q10CaL, Qpow)
                                                * (FP_LITERAL(1.) - f_Ca_B) * d * f * ibarca_j;
                ...
                const cellmodel_float_t df_Ca_B_dt =
                        FP_LITERAL(-0.012) * f_Ca_B
                        + (FP_LITERAL(1.7) - FP_LITERAL(1.7) * f_Ca_B) * cd;
                ...
                const cellmodel_float_t s3_sl =
                        ...
                        + KmCai * (Nao * Nao * Nao)
                                  * (FP_LITERAL(1.)
                                     + (Na_i * Na_i * Na_i) / (KmNai * KmNai * KmNai))
                ...
                const cellmodel_float_t I_pCa = g_pCa * lambda_c_e * pow(Q10SLCaP, Qpow)
                                                * (csl * csl) / ((KmPCa * KmPCa) + (csl * csl));
                ...
                const cellmodel_float_t xf_inf =
                        FP_LITERAL(1.0)
                        / (FP_LITERAL(1.)
                           + FP_LITERAL(5956538.01318461) * Exp(V_m / FP_LITERAL(5.)));
                const cellmodel_float_t tau_xf =
                        FP_LITERAL(1900.)
                        / (FP_LITERAL(1.)
                           + FP_LITERAL(4.48168907033806) * Exp(V_m / FP_LITERAL(10.)));
                ...
                const cellmodel_float_t J_sl_c =
                        alpha_sl_c * lambda_c_i * lambda_diff * (-cc + csl);
                ...
                const cellmodel_float_t p =
                        FP_LITERAL(1.0)
                        / (FP_LITERAL(1.) + (K_RyR * K_RyR * K_RyR) / (cd * cd * cd));
                ...
                const cellmodel_float_t J_c_b =
                        Vc
                        * (-k_off_c * bc + k_on_c * (-bc + B_tot_c * lambda_B * lambda_B_c) * cc);
                const cellmodel_float_t J_d_b =
                        Vd
                        * (-k_off_d * bd + k_on_d * (-bd + B_tot_d * lambda_B * lambda_B_c) * cd);
                const cellmodel_float_t J_s_b =
                        Vs * (-k_off_s * bs + k_on_s * (-bs + B_tot_s * lambda_B) * cs);
                const cellmodel_float_t J_sl_b =
                        Vsl
                        * (-k_off_sl * bsl
                           + k_on_sl * (-bsl + B_tot_sl * lambda_B * lambda_B_c) * csl);
                ...
                const cellmodel_float_t dcc_dt =
                        (FP_LITERAL(1.0) * J_d_c + FP_LITERAL(1.0) * J_sl_c
                         - FP_LITERAL(1.0) * J_SERCA - FP_LITERAL(1.0) * J_c_b)
                        / Vc;
                states[STATE_cc * padded_num_cells + i] = dt * dcc_dt + cc;
                const cellmodel_float_t dcd_dt = (FP_LITERAL(1.0) * J_CaL - FP_LITERAL(1.0) * J_d_b
                                                  - FP_LITERAL(1.0) * J_d_c)
                                                 / Vd;
                states[STATE_cd * padded_num_cells + i] = dt * dcd_dt + cd;
                const cellmodel_float_t dcsl_dt =
                        (FP_LITERAL(1.0) * J_RyR + FP_LITERAL(1.0) * J_e_sl
                         - FP_LITERAL(1.0) * J_sl_b - FP_LITERAL(1.0) * J_sl_c)
                        / Vsl;
                states[STATE_csl * padded_num_cells + i] = dt * dcsl_dt + csl;
                const cellmodel_float_t dcs_dt = (FP_LITERAL(1.0) * J_n_s - FP_LITERAL(1.0) * J_RyR
                                                  - FP_LITERAL(1.0) * J_s_b)
                                                 / Vs;
                ...
                const cellmodel_float_t I_tot = I_CaL + I_K1 + I_KATP + I_Kr + I_Ks + I_Na + I_NaCa
                                                + I_NaK + I_NaL + I_bCa + I_bCl + I_f + I_pCa
                                                + I_to;
...
#pragma clang loop vectorize(assume_safety)
            ...
            for (long i = v; i < v + v_len; i++) {
                ...
                const cellmodel_float_t dibarca_j_dcd =
                        FP_LITERAL(1.36) * Frdy * g_CaL * FoRT * V_m
                ...
                const cellmodel_float_t dI_pCa_dcsl =
                        FP_LITERAL(-2.) * g_pCa * lambda_c_e * pow(Q10SLCaP, Qpow)
                                * (csl * csl * csl)
                                / (((KmPCa * KmPCa) + (csl * csl))
                                   * ((KmPCa * KmPCa) + (csl * csl)))
                ...
                const cellmodel_float_t dJ_leak_dcsl =
                        -alpha_RyR * gamma_RyR * lambda_RyR * lambda_c_i;
                ...
                const cellmodel_float_t dKa_sl_dcsl =
                        ...
                        / (((FP_LITERAL(1.) + (Kdact * Kdact) / (csl * csl))
                            * (FP_LITERAL(1.) + (Kdact * Kdact) / (csl * csl)))
                           * (csl * csl * csl));
                const cellmodel_float_t deca_sl_dcsl =
                        FP_LITERAL(-1.) / (FP_LITERAL(2.) * FoRT * csl);
                ...
                const cellmodel_float_t dJ_RyR_active_dcs =
                        alpha_RyR * lambda_RyR * lambda_c_i * p * r_RyR;
                const cellmodel_float_t dJ_leak_dcs =
                        alpha_RyR * gamma_RyR * lambda_RyR * lambda_c_i;
                ...
                const cellmodel_float_t ds2_sl_dV_m = (Nao * Nao * Nao) * (FP_LITERAL(-1.) + nu)
                                                      * FoRT * csl
                                                      * Exp((FP_LITERAL(-1.) + nu) * FoRT * V_m);
                const cellmodel_float_t dI_NaCa_dV_m =
                        g_NaCa * lambda_c_e * pow(Q10NCX, Qpow) * (-ds2_sl_dV_m + ds1_sl_dV_m)
                                * Ka_sl
                                / ((FP_LITERAL(1.)
...
// clang-format off
const struct cellmodel model_JT21_multistep_cell_time_cell = {
