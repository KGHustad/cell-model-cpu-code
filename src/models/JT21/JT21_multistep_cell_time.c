...
#ifdef VECTOR_LENGTH
#pragma omp parallel for simd aligned(states : CELLMODEL_STATES_ALIGNMENT_BYTES) simdlen(VECTOR_LENGTH)
#else
...
#pragma omp parallel for
    ...
    for (long i = 0; i < num_cells; i++) {
        ...
        const cellmodel_float_t lambda_RyR = parameters[PARAM_lambda_RyR * padded_num_cells + i];
        ...
        const cellmodel_float_t J_SERCA_bar = parameters[PARAM_J_SERCA_bar * padded_num_cells + i];
        ...
        const cellmodel_float_t lambda_B_c = parameters[PARAM_lambda_B_c * padded_num_cells + i];
        ...
        const cellmodel_float_t alpha_sl_c = parameters[PARAM_alpha_sl_c * padded_num_cells + i];
        const cellmodel_float_t lambda_c_d = parameters[PARAM_lambda_c_d * padded_num_cells + i];
        const cellmodel_float_t lambda_c_i = parameters[PARAM_lambda_c_i * padded_num_cells + i];
        const cellmodel_float_t lambda_diff = parameters[PARAM_lambda_diff * padded_num_cells + i];
        ...
        const cellmodel_float_t lambda_c_e = parameters[PARAM_lambda_c_e * padded_num_cells + i];
        ...
        const cellmodel_float_t stim_period = parameters[PARAM_stim_period * padded_num_cells + i];
        const cellmodel_float_t stim_start = parameters[PARAM_stim_start * padded_num_cells + i];
...
// clang-format off
const struct cellmodel model_JT21_multistep_cell_time = {
