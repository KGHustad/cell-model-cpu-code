...
void init_parameters_values(double *parameters)
{
    ...
    parameters[PARAM_is_stimulated] = 0.0;
                        ...
                        const double *__restrict parameters)
{
    ...
    // Assign parameters
    const double celltype = parameters[PARAM_celltype];
    ...
    const double i_CaL_lim_delta = parameters[PARAM_i_CaL_lim_delta];
    ...
    const double is_stimulated = parameters[PARAM_is_stimulated];
                          ...
                          const double *__restrict parameters)
{
    ...
    states[STATE_Xr1] = (expm1(dt * dXr1_dt_linearized)) * dXr1_dt / dXr1_dt_linearized + Xr1;
    ...
    states[STATE_Xr2] = (expm1(dt * dXr2_dt_linearized)) * dXr2_dt / dXr2_dt_linearized + Xr2;
    ...
    states[STATE_Xs] = (expm1(dt * dXs_dt_linearized)) * dXs_dt / dXs_dt_linearized + Xs;
    ...
    states[STATE_f2] = (expm1(dt * df2_dt_linearized)) * df2_dt / df2_dt_linearized + f2;
...
void step_RL_singlecell(double *__restrict states, const double t, const double dt,
                        const double *__restrict parameters)
{
